...
export default {
  ...
  methods: {
    ShowUser: function () {
      let user = this.users.filter((item) => item.id == this.$route.params.pathMatch)[0];
      this.user = user;
      this.$root.$on("send", (user, id) => {
        let match = this.users.filter((item) => item.id == id)[0];
        match.name = user.name
      });
    },
  },
  // Сработает как только прогрузится страница
  mounted() {
    this.ShowUser();
  },
};
